...
- `src/main/`：Electron 生命周期与窗口；`agent/`、`document/`、`logging/`、`presentation/`、`project/`、`version-control/` 分别承载 Agent、本地 Word 读取、诊断、演示文稿、项目存储和自动版本服务及其 IPC。
...
- `docs/`：开发指南、Agent、文档读取与导出的当前说明；入口见 [README.md](README.md)。
...
项目总入口 `README.md` 保留在仓库根目录；其余项目说明、开发指南、样本说明和第三方声明统一放在 `docs/`。文档只维护当前实现、使用方式与有效限制，不保留实施计划、阶段验收流水或历史测量记录。新增或移动文档时同步更新链接和构建引用，除根目录 README 外，不在根目录或源码、脚本目录重复维护说明文档。
...
- `pnpm test`：运行 Vitest 单元测试和 Electron 渲染端测试。
- `pnpm format`：使用 Prettier 格式化仓库。
- `pnpm format:check`：检查仓库格式但不写入文件。
- `pnpm package`：校验、构建并生成当前平台安装包。
- `pnpm package:mac` / `pnpm package:win`：在 `out/release/` 生成平台安装包。

## 编码风格与命名约定

遵循现有 TypeScript 风格：两空格缩进、单引号、不写分号，多行结构按需保留尾逗号。保持严格类型，避免 `any`，并在 IPC 边界校验输入。React 组件和类型使用 `PascalCase`，函数与变量使用 `camelCase`，文件名使用短横线命名，例如 `config-store.ts`。使用 Prettier 格式化，并通过项目的 TypeScript 配置校验类型。
...
项目使用 Vitest。验证逻辑、共享工具及 IPC 边界行为应补充针对性测试；Agent 工具、revision 冲突、演示文稿转换、质量预检、日志脱敏及 Skill 打包契约属于重点覆盖范围。测试套件以导出单元命名，测试用例描述可观察行为。项目未设置强制覆盖率阈值，但新增行为和缺陷修复仍应配套测试。本地提交前运行 `pnpm test` 和 `pnpm format:check`。
