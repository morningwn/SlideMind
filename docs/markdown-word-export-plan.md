...
日期：2026-09-16。状态：P0 验证持续进行；P1 已完成核心服务与 IPC；P2 已完成界面与打包实现；P3 已开始自动化验收，真机安装与 Word 视觉验收待完成。

## 1. 目标与决策

为项目内 `.md`、`.markdown` 文档提供离线导出 `.docx` 的能力，保留可编辑的标题、正文、列表和表格，并提供统一中文文档样式。

本方案以**随应用内置各平台 Pandoc 可执行文件**为基线，贴合此前关于内置方案的讨论。P2 已完成正式接入实现，但不代表安装包体积增量或正式发布已经通过 P3 验收；三目标安装包实测后仍需据实决策。
...
| 随包对应源码归档                       | 9,018,272 bytes   | 9,018,272 bytes   | 9,018,272 bytes   |
| 安装包压缩增量（分别记录 DMG/ZIP/NSIS） | 待测              | 待测              | 待测              |
| 安装后体积增量                          | 待测              | 待测              | 待测              |
| 文本 / 图表样本转换耗时与峰值内存       | 已测，见 P0 记录  | 待目标机          | 待目标机          |

安装包增量需包含 Pandoc、样式、通知文件和对应源码归档，并与已有 Tika/Java 增量分开。用户尚未给出可接受体积阈值，P0 报告提交后据实决策，不自行假设“几十 MB 可以接受”。
...
实施状态（2026-09-16）：已完成共享契约、受限 preload/IPC、Pandoc 两阶段运行时、AST 内容策略、PNG/JPEG 项目资源授权、原子输出与目标变更检测、项目内版本提交、窗口及应用退出取消，并加入单元、IPC 和本机真实 Pandoc 集成测试。后续 P2 界面入口和正式打包资源接入也已完成。

### P2：界面与打包

接入 Markdown 顶部按钮、当前快照导出和提示；整合目标平台打包与样式文件，补充渲染端测试及截图。更新 README 的真实用户行为说明。

实施状态（2026-09-16）：已接入 Markdown 当前快照导出、取消/失败/成功/警告提示和单标签重复导出保护；转换不保存源文件，冲突状态下仍可导出本地编辑版本。已加入渲染端覆盖与截图产物。打包链路按目标架构准备、复制和校验 Pandoc 3.11、`reference.docx`、许可证通知及校验固定的对应源码归档，并通过组合 hook 保留 Tika 打包。三目标真实安装与 Word 视觉验收归入 P3。
...
实施状态（2026-09-16）：已开始补齐自动化门禁。真实 Pandoc 集成测试已从 ZIP 签名检查提升为 DOCX 内部标题、起始编号、表格、超链接和媒体关系检查；子进程异常、任务隔离、退出清理、失效句柄、编码路径、已有文件保护、项目内外版本行为和错误运行时版本已有针对性覆盖。当前机器的单元测试、类型检查和渲染端导出流程已分别通过。三目标安装包、离线运行、安装包体积和 macOS/Windows 真实 Word 视觉检查仍未通过，不得据此标记 P3 完成。进度与复现命令见 [P3 验收记录](markdown-word-export-p3-validation.md)。
