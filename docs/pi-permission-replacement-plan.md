...
- 会话使用内存 `SettingsManager`，受控资源加载器不读取设置文件，启用 Pi 原生压缩；模型运行时使用内存凭据存储，由应用安全存储注入 Key。
...
- Pi 0.85.1 的固定版本补丁公开内联工厂加载函数，`managed-resources.ts` 直接调用该入口，不再推导包内路径。SDK 升级必须重新运行注册、reload 和会话集成测试；生产构建通过不等于所有平台安装包运行通过。
