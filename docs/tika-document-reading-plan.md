...
状态：P0 实施中；macOS arm64 首轮结果因 Server 无本地调用方认证而阻断，详见 `docs/tika-p0-validation.md`。尚未进入读取服务或安装包实现。
...
- 本地监听不等于鉴权。原型必须验证选定 Server 版本能否提供有效请求鉴权，以及浏览器来源限制；随机端口和关闭 CORS 都不单独作为鉴权措施。
- 若官方 Server 无法满足本地访问控制要求，暂停该方案的产品化，提出本地 IPC/CLI 或小型 Java 适配程序的替代提案，不静默扩大架构。
...
- [ ] 验证本地鉴权、浏览器来源约束、外部资源访问关闭。
- [ ] 验证取消、超时、崩溃后的进程树回收和服务恢复。
- [ ] 用中文 DOC/DOCX 及复杂内容样本比较输出，记录丢失或混合的内容。
- [ ] 测量冷启动、解析耗时、峰值内存与资源包大小。

产物：可复现验证脚本、样本预期、验证报告、版本与配置清单。未通过本地访问控制或可靠终止验证，不进入打包交付；先形成替代方案。

### P1：读取服务

- [ ] 定义共享契约与边界校验。
- [ ] 实现运行时、客户端、文件快照、队列、缓存和游标。
- [ ] 完成错误映射、输出限制和日志脱敏。
- [ ] 完成服务层单元测试及固定版本 Tika 集成测试。

### P2：Agent 接入

- [ ] 注册工具与权限规则，修正扩展数量校验。
- [ ] DOC/DOCX 引用自动引导至 `document_read`。
- [ ] 验证已有 PPTX、原生演示文稿、文本读取路由不回归。
- [ ] 验证工具活动显示、取消和失败信息。
- [ ] 更新 README，必要时同步内置 Skill 契约。
...
| 风险 | 处理方式 |
|---|---|
| 官方服务无法满足本地访问控制 | P0 阻断产品化，评估本地 IPC/CLI 或小型 Java 适配程序 |
...
实施前待验证而非已确认的事项：精确 Tika/Java 版本、本地鉴权机制、HTTP 取消与解析终止行为、输出截断标识、真实结构保留程度、包体和性能预算。任何需要改为远程服务、运行时在线下载或扩大格式范围的变化须单独说明影响。
