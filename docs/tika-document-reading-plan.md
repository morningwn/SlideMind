...
状态：P0 仍有跨平台、超时和峰值资源验证未完成；P1 读取服务与 P2 Agent 接入已完成，macOS arm64 固定版本集成测试已通过；P3 已开始，实现了按架构资源准备、打包注入和安装包目录冒烟检查，目标机安装验证仍未完成。
...
- [ ] 已将 Tika LICENSE/NOTICE、Temurin NOTICE/legal 纳入按架构资源校验；日志和诊断包仍需目标机复核。
