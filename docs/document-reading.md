...
`document_read` 仅用于项目内 DOC/DOCX 的正文和筛选后元数据，文件引用自动路由到该工具。PPTX、原生演示文稿和普通文本继续使用各自的读取工具。不提供页面视觉、准确页码、OCR、嵌入附件递归、密码解密或复杂表格的无损结构。
...
| 项目                 | 当前配置                                                        |
| -------------------- | --------------------------------------------------------------- |
| 输入文件             | 非空，最多 30 MiB；后缀与检测 MIME 均须支持                     |
| 启动等待             | 30 秒                                                           |
...
- 仅加载 Office/OOXML 解析器，限制嵌入递归、XML 深度、压缩展开和输出；关闭 CORS、Pipes 请求端点及每请求配置，不允许 Agent 指定服务地址、Java 参数或解析配置。
