...
状态：P0 阻断；macOS arm64 首轮已执行，macOS x64 与 Windows x64 待补。当前候选 Server 架构不得进入 P1/P3 产品化。
...
[推论] 当前“官方 Server 直接监听随机回环端口”的候选架构未满足规划中的本地访问控制门槛。在证明 mTLS 可由应用安全、跨平台且无需额外高成本运行时完成，或改用不暴露 TCP 端口的本地 IPC/CLI 适配前，不得进入产品打包。

macOS arm64 实测的其余探针通过：CORS 未授予非可信来源、每请求配置返回 403、Pipes 端点返回 404、DOC/DOCX 内容与 MIME 检测通过、客户端取消后服务仍响应、被杀死的解析 fork 可替换、退出后所属进程树无残留。唯一阻断性失败是未认证 `/version` 返回 200，这与官方安全模型一致。
...
1. 本地 TCP 端点没有调用方认证，是当前首要阻断项。随机端口、随机 Server ID 与关闭 CORS 均不是认证。
...
建议并行评估两个替代面：每次请求启动 `tika-app` 的 stdin/stdout CLI；或实现只接受受限二进制协议的小型 Java 子进程适配器。两者都避免向本机暴露无认证 HTTP 端点，代价是启动性能或适配器维护成本，需要后续原型数据决策。
