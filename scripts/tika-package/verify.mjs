...
for (const platform of expectedPlatforms) {
  const manifest = manifests.find((path) => path.includes(`${join('tika-runtime', platform, 'prepared-runtime.json')}`))
  if (!manifest) throw new Error(`Packaged Tika runtime was not found for ${platform}`)
  ...
  if (platform === `${process.platform}-${process.arch}`) {
    ...
    await run(runtime.javaBinary, ['-jar', runtime.tikaJar, '--help'], { cwd: runtime.tikaDirectory })
...
async function verifyDocumentExtraction(runtime) {
  ...
  const child = spawn(runtime.javaBinary, [
    '-Xmx256m',
    `-Djava.io.tmpdir=${workDirectory}`,
    '-jar', runtime.tikaJar,
    '-c', runtime.configPath,
    '-h', '127.0.0.1',
    '-p', String(port),
    '-i', `slidemind-package-${randomUUID()}`
  ], {
  ...
  try {
    ...
    const fixture = await readFile(join(repositoryRoot, 'scripts/tika-p0/fixtures/simple-content.docx'))
    ...
    const content = Array.isArray(entries)
      ...
      : ''
    for (const expected of ['SlideMind 中文文档读取样本', 'SLIDEMIND TIKA P0 END']) {
      if (!content.includes(expected)) throw new Error(`Packaged Tika extraction missed: ${expected}`)
    }
  } finally {
...
async function waitForServer(port, child) {
  ...
  while (Date.now() < deadline) {
    if (child.exitCode !== null) throw new Error(`Packaged Tika exited with ${child.exitCode}`)
    try {
