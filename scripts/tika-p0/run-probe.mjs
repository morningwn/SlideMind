...
const reportPath = resolve(
  readOption('--report') ?? join(repositoryRoot, '.local/tika-p0-probe.json'),
)
...
try {
  ...
  report.measurements.coldStartMillis = Math.round(
  ...
  const ownedPids = report.measurements.processTree?.processes?.map(
    (process) => process.pid,
  ) ?? [child.pid]
  ...
  addTest(
    'owned process tree is reclaimed on shutdown',
...
async function probeAccessControls() {
  ...
  addTest(
    'version endpoint is pinned',
    version.status === 200 && version.text.includes(prepared.tikaVersion),
    {
  ...
  addTest(
    'unauthenticated loopback access matches the accepted threat model',
    unauthenticated.status === 200,
    {
      status: unauthenticated.status,
      acceptedRisk:
        'Native processes running as the same OS user are trusted callers; shared-host isolation is out of scope.',
    },
  )
  ...
  addTest(
    'untrusted browser origin receives no CORS grant',
  ...
  const config = await request('/tika/config/text', {
    method: 'POST',
    body: new Uint8Array(),
  })
  addTest(
    'per-request parser configuration is disabled',
    config.status === 403 || config.status === 404,
    {
  ...
  addTest(
    'fetcher and emitter endpoint is disabled',
    pipes.status === 403 || pipes.status === 404,
    {
      status: pipes.status,
    },
  )
...
async function probeSamples() {
  ...
  for (const sample of expectations.samples) {
    ...
    const missing = sample.requiredText.filter(
    ...
    addTest(
      `sample ${sample.file} extracts required text`,
      detected.status === 200 &&
        parsed.status === 200 &&
        missing.length === 0 &&
        outOfOrder.length === 0,
...
async function probeRecovery() {
  ...
  addTest(
    'server remains responsive after client cancellation',
...
async function probeForkCrashRecovery() {
  if (process.platform === 'win32') {
    addTest('server replaces a crashed parse fork', false, {
      pending: 'Windows runner not implemented',
    })
    return
  }
  ...
  if (!worker) {
    addTest('server replaces a crashed parse fork', false, {
      finding: 'No parse fork was observable',
    })
    return
  }
  ...
  let response = await request('/rmeta/text', {
    method: 'PUT',
    headers: {
      'Content-Disposition': 'attachment; filename="simple-content.docx"',
    },
    body: bytes,
  })
  ...
  const replacement = after.processes.find(
    (process) => process.ppid === child.pid && process.pid !== worker.pid,
  )
  addTest(
    'server replaces a crashed parse fork',
    response.status === 200 && Boolean(replacement),
    {
...
async function waitForServer(port, processHandle) {
  ...
  while (Date.now() < deadline) {
    if (processHandle.exitCode !== null)
      throw new Error(
        `Tika exited during startup with ${processHandle.exitCode}`,
      )
    try {
      const response = await fetch(`http://127.0.0.1:${port}/version`, {
        signal: AbortSignal.timeout(500),
      })
      if (response.ok && (await response.text()).includes(prepared.tikaVersion))
        return
    } catch {}
...
async function processTree(parentPid) {
  if (process.platform === 'win32')
    return {
      supported: false,
      reason: 'Windows process-tree measurement pending native runner',
    }
  return new Promise((resolvePromise) => {
    ...
    ps.once('exit', () => {
      ...
      resolvePromise({
        ...
        totalRssKiB: tree.reduce((sum, row) => sum + row.rssKiB, 0),
        processes: tree,
      })
    })
  })
