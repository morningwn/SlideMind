...
const timeout = setTimeout(() => {
  console.error('Renderer suite timed out')
  app.exit(1)
}, 180_000)
app.whenReady().then(async () => {
  ...
  window.webContents.on('page-title-updated', (_event, title) => {
    if (title === 'SlideMind renderer tests - conversation archive ready') {
      markdownExportScreenshot = window.webContents
        .capturePage()
        .then((image) =>
  ...
  window.webContents.on('console-message', (details) => {
    ...
    if (details.level === 'error' || details.message.startsWith('[test]'))
  ...
  try {
    ...
    const result = await window.webContents.executeJavaScript(
      'window.rendererTestResult',
    )
    await markdownExportScreenshot
    if (errors.length)
      result.error = [result.error, ...errors].filter(Boolean).join('\n')
    await writeFile(
    ...
    await writeFile(
      join(output, result.error ? 'failure.png' : 'pptist.png'),
