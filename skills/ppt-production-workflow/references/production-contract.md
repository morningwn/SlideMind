...
- `intake → evidence-ready`：先读取引用材料并区分事实、推论、建议和待确认项。
...
1. `slides_create`
2. `slides_read`
3. `slides_write`，使用刚读取的 revision 和全部页面
4. `slides_read`，完成结构复核
5. 必要时重复“读取 → 修复写入 → 回读”
6. 用户要求 PPTX 时再 `slides_export`
7. 可选 `pptx_read` 回读导出内容，验证语义而非渲染
