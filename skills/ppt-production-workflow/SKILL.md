...
大纲 Markdown、`.slides.json`、按需导出的 `.pptx`、来源记录、质量报告及生成或下载的素材都必须位于该目录下；允许使用目录内的 `assets/`、`sources/` 等子目录。临时文件不作为产物保留，用户提供的原始材料也不移动或复制，除非用户明确要求。
...
明确受众、演示目的、期望行动、使用场景、材料范围、语言、品牌或模板约束，以及交付物是 Markdown、`.slides.json`、`.pptx` 中的哪些文件。按“用户目录 → 明确文件的父目录 → 主题生成目录”的优先级确定统一产物目录。只有会改变核心结论、受众行动或不可逆写入范围的缺口才需要追问；其余使用保守假设并显式说明。
...
需要新文件时，在已经由 Markdown 写入创建的统一产物目录中调用 `slides_create`。`.slides.json` 路径必须位于该目录；任何 `slides_write` 之前都必须立即调用 `slides_read` 并使用其最新 revision。
...
只有用户要求 PPTX 时才调用 `slides_export`，并显式把输出路径设为统一产物目录中的 `.pptx`，不要依赖默认输出位置。导出后可用 `pptx_read` 回读语义内容，但这不能证明字体替换、文字溢出、图片裁切或 PowerPoint 兼容性已经通过。当前没有真实渲染检查工具时，必须把视觉验收明确标为待在 PPTist 或导出的 PowerPoint 中确认。
