...
export async function persistenceTests(): Promise<string[]> {
  ...
  try {
    ...
    await delay(350)
    assert(
      Number(saved.length) === 0,
      'Disabled persistence wrote over unloaded metadata',
    )
    root.render(
      createElement(Harness, { title: 'intermediate', enabled: true }),
    )
    await delay(50)
    root.render(createElement(Harness, { title: 'latest', enabled: true }))
    await delay(350)
    assert(
      Number(saved.length) === 1 &&
        saved[0].conversations[0].title === 'latest',
      'Debounce failed to save latest metadata once',
    )
    root.render(createElement(Harness, { title: 'latest', enabled: true }))
    await delay(350)
    assert(
      Number(saved.length) === 1,
      'Unchanged snapshot caused another write',
    )
    failure = true
    root.render(createElement(Harness, { title: 'retry', enabled: true }))
    await delay(350)
    assert(
      String(error) === 'disk unavailable',
      'Save failure was not surfaced',
    )
    failure = false
    ...
    assert(
      String(error) === '' &&
        saved.at(-1)?.conversations[0].title === 'recovered',
      'Save recovery failed',
    )
    root.render(
      createElement(Harness, { title: 'unmount flush', enabled: true }),
    )
    await delay(30)
    root.unmount()
    await delay(30)
    assert(
      saved.at(-1)?.conversations[0].title === 'unmount flush',
      'Unmount lost pending metadata',
    )
    return [
      'Persistence hook: loading gate, debounce, snapshot deduplication, failure recovery and unmount flush',
    ]
  } finally {
    root.unmount()
    host.remove()
