...
普通分析、问答、总结和关键词概括默认直接在对话中回答；引用文件仅表示输入材料，不意味着保存文档或制作 PPT。文件交付取决于用户要求，历史课件与旧工作流不构成新任务授权。“只回答、不生成文件”的要求对后续相关问题持续有效，直到用户明确改变交付方式。

复杂任务由内置 `task-workflow` 维护 `task-status.md`；简单问答的一次读取失败不自动触发文件工作流，用户明确要求不写文件时仅在对话中记录。完整演示由 `ppt-production-workflow` 按证据、大纲、逐页文案、设计、草稿、质量审查和按需导出推进，在主题目录维护 `workflow-status.md` 与产物。完整交付默认连续推进，用户要求逐阶段确认时使用审阅模式；任务记录不授权额外交付文件。

指定材料无法读取时，Agent 应说明未读到的范围；没有运行时恢复依据时不反复重试，替代来源不能作为已核验的指定材料。精确引文必须来自实际读取到的内容，文本解释与教材原意分开说明。
