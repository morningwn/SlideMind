...
SlideMind 是一个面向 macOS 与 Windows 的文案创作桌面工作区。应用将项目文件、Markdown 编辑、PPTist 可编辑演示、AI 制作流程、自动版本历史与本地诊断整合在同一个 Electron 应用中。
...
- Agent 按用户要求完成材料整理、分析、写作、改写与审校；PPT 是可选交付物，普通文案不会自动进入演示制作流程。复杂或可行性不明确的任务先说明分步方案和验证方式，通过内置 `task-workflow` 在项目内维护 `task-status.md`，记录任务内容、步骤状态、产物、验证结果、阻塞和下一步；恢复时先读取记录与产物。简单任务直接完成，分步不意味着强制逐步确认，阶段成果不等于整体完成。用户明确要求不写文件时仅在对话中记录。
...
- 每个完整演示任务在项目内使用一个稳定的主题产物目录，并通过 `workflow-status.md` 记录执行模式、阶段状态、产物路径、假设、待确认项和下游失效状态。大纲、逐页文案、设计规范、`.slides.json`、质量报告、素材与按需导出的 `.pptx` 均归入该目录。
