...
export function WorkspaceResizeHandle({ orientation, label, value, min, max, onChange }: WorkspaceResizeHandleProps): React.JSX.Element {
  const [dragOrigin, setDragOrigin] = useState<{ coordinate: number; value: number } | null>(null)
  const vertical = orientation === 'vertical'
  const update = (next: number): void => onChange(Math.min(max, Math.max(min, next)))
  ...
  return (
    <div
      ...
      onPointerDown={(event) => {
        ...
        setDragOrigin({ coordinate: vertical ? event.clientX : event.clientY, value })
      }}
      onPointerMove={(event) => {
        if (dragOrigin) update(dragOrigin.value + (vertical ? event.clientX : event.clientY) - dragOrigin.coordinate)
      }}
      onPointerUp={(event) => {
      ...
      onKeyDown={(event) => {
        ...
        event.preventDefault()
        update(event.key === 'Home' ? min : event.key === 'End' ? max : value + (event.key === decrease ? -10 : 10))
      }}
    >
