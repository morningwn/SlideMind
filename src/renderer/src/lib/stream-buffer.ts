...
export function createStreamBuffer(consume: (events: AgentStreamEvent[]) => void) {
  ...
  function drain(): AgentStreamEvent[] {
    clearTimeout(timer)
    timer = undefined
    const events = [...pending.values()].flatMap((requests) => [...requests.values()])
    pending.clear()
    return events
  }
  ...
  return {
    push(event: AgentStreamEvent): void {
      ...
      requests.set(event.requestId, { ...event, delta: (previous?.delta ?? '') + event.delta })
...
export function applyStreamEvents<T extends {
  id: string
  messages: Array<{ id: string; text: string; isStreaming?: boolean }>
}>(conversations: T[], events: AgentStreamEvent[]): T[] {
  return conversations.map((conversation) => {
    const deltas = new Map(events.filter((event) => event.conversationId === conversation.id)
    ...
    return {
      ...
      messages: conversation.messages.map((message) => {
        ...
        return delta && message.isStreaming ? { ...message, text: message.text + delta } : message
      })
    }
  })
}
