...
describe('projectFileDisplayKind', () => {
  it.each([
    [
      'a directory with a file-like name',
      { kind: 'directory', path: 'notes.md' },
      'directory',
    ],
    [
      'a presentation source file',
      { kind: 'file', path: 'deck.slides.json' },
      'presentation',
    ],
    [
      'an uppercase presentation suffix',
      { kind: 'file', path: 'DECK.SLIDES.JSON' },
      'presentation',
    ],
    ['a Markdown file', { kind: 'file', path: 'notes.md' }, 'markdown'],
    [
      'a long Markdown suffix',
      { kind: 'file', path: 'notes.MARKDOWN' },
      'markdown',
    ],
...
describe('isOpenableProjectFile', () => {
  it.each(['presentation', 'markdown', 'text', 'image'] as const)(
    'opens supported %s files',
    (kind) => {
