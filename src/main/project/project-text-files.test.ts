...
describe('ProjectTextFileStore', () => {
  ...
  it('rejects unsupported, binary, oversized and internal files', async () => {
    ...
    await expect(store.read(projectPath, 'data.json')).rejects.toThrow(
      '仅支持 Markdown 和 TXT',
    )
    await expect(store.read(projectPath, 'binary.txt')).rejects.toThrow(
      '二进制内容',
    )
    await expect(store.read(projectPath, 'large.md')).rejects.toThrow(
      '超过 2 MiB',
    )
    await expect(
      store.read(projectPath, '.slideMind/private.txt'),
  ...
  it('rejects paths outside the project and symbolic links', async () => {
    ...
    const outsidePath = await mkdtemp(
      join(tmpdir(), 'slidemind-text-files-outside-'),
    )
    ...
    await expect(store.read(projectPath, '../outside.md')).rejects.toThrow(
      '超出项目范围',
    )
    await expect(store.read(projectPath, 'linked.md')).rejects.toThrow(
      '符号链接',
    )
  })
  ...
  it('loads only safe project-relative images for Markdown previews', async () => {
    ...
    await writeFile(
      join(projectPath, 'images', 'cover.png'),
      Buffer.from([1, 2, 3]),
    )
    const store = new ProjectTextFileStore()

    await expect(
      store.readPreviewAsset(
        projectPath,
        'docs/notes.md',
        '../images/cover.png',
      ),
  ...
  it('rejects unsupported, oversized and symbolic-link image previews', async () => {
    const projectPath = await mkdtemp(join(tmpdir(), 'slidemind-image-files-'))
    const outsidePath = await mkdtemp(
      join(tmpdir(), 'slidemind-image-files-outside-'),
    )
    await writeFile(join(projectPath, 'vector.svg'), '<svg />')
    await writeFile(
      join(projectPath, 'large.png'),
      Buffer.alloc(20 * 1024 * 1024 + 1),
    )
    await writeFile(join(outsidePath, 'outside.png'), Buffer.from([1, 2, 3]))
    await symlink(
      join(outsidePath, 'outside.png'),
      join(projectPath, 'linked.png'),
    )
    const store = new ProjectTextFileStore()

    await expect(store.readImage(projectPath, 'vector.svg')).rejects.toThrow(
      '仅支持 PNG',
    )
    await expect(store.readImage(projectPath, 'large.png')).rejects.toThrow(
      '超过 20 MiB',
    )
    await expect(store.readImage(projectPath, 'linked.png')).rejects.toThrow(
      '符号链接',
    )
  })
  ...
  it('loads only real PNG and JPEG bytes for Markdown Word export', async () => {
    const projectPath = await mkdtemp(
      join(tmpdir(), 'slidemind-export-images-'),
    )
    await mkdir(join(projectPath, 'docs'))
    await mkdir(join(projectPath, 'images'))
    await writeFile(join(projectPath, 'docs', 'notes.md'), '# Notes')
  ...
  it('rejects remote, absolute, escaping and symbolic-link export images', async () => {
    ...
    const outsidePath = await mkdtemp(
      join(tmpdir(), 'slidemind-export-images-outside-'),
    )
    ...
    await expect(
      readMarkdownExportImage(projectPath, 'notes.md', '/etc/passwd'),
    ).rejects.toThrow('当前项目内')
    await expect(
    ...
    await expect(
      readMarkdownExportImage(projectPath, 'notes.md', 'linked.png'),
