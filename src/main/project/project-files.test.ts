...
describe('listProjectDirectory', () => {
  ...
  it('lists recursively referenceable files while excluding generated and internal paths', async () => {
    ...
    await mkdir(join(projectPath, 'node_modules', 'dependency'), {
      recursive: true,
    })
    await mkdir(join(projectPath, '.slideMind'))
    ...
    await writeFile(
      join(projectPath, 'node_modules', 'dependency', 'index.js'),
      'module',
    )
    ...
    expect(await listProjectFiles(projectPath)).toEqual([
      {
        kind: 'file',
        name: 'brief.md',
        path: join('docs', 'nested', 'brief.md'),
      },
  ...
  it('rejects paths outside the project', async () => {
    ...
    await expect(
      listProjectDirectory(projectPath, projectPath),
  ...
  it('creates project directories and Markdown documents', async () => {
    ...
    await expect(
      createProjectMarkdownFile(projectPath, join('docs', 'notes.md')),
    ).resolves.toEqual({
      kind: 'file',
      name: 'notes.md',
    ...
    await expect(
      readFile(join(projectPath, 'docs', 'notes.md'), 'utf8'),
    ).resolves.toBe('# 未命名文档\n')
  })
  ...
  it('rejects unsafe, duplicate and unsupported creation targets', async () => {
    ...
    const outsidePath = await mkdtemp(
      join(tmpdir(), 'slidemind-files-outside-'),
    )
    await mkdir(join(projectPath, '.slideMind'))
    ...
    await expect(
      createProjectDirectory(projectPath, '../outside'),
    ).rejects.toThrow('超出项目范围')
    await expect(
      createProjectDirectory(projectPath, '.slideMind/private'),
    ).rejects.toThrow('内部目录')
    await expect(
      createProjectDirectory(projectPath, 'existing'),
    ).rejects.toThrow('同名')
    await expect(
      createProjectMarkdownFile(projectPath, 'notes.txt'),
    ).rejects.toThrow('Markdown 文件必须使用')
    await expect(
      createProjectMarkdownFile(projectPath, join('linked', 'notes.md')),
  ...
  it('renames a regular file without leaving the project', async () => {
    ...
    await expect(
      readFile(join(projectPath, 'notes', 'final.md'), 'utf8'),
    ).resolves.toBe('Draft')
    await expect(
      access(join(projectPath, 'notes', 'draft.md')),
    ).rejects.toThrow()
  })
  ...
  it('deletes regular files but rejects directories and internal files', async () => {
    ...
    await expect(
      deleteProjectFile(projectPath, 'obsolete.md'),
    ).resolves.toBeUndefined()
    await expect(access(join(projectPath, 'obsolete.md'))).rejects.toThrow()
    await expect(deleteProjectFile(projectPath, 'folder')).rejects.toThrow(
      '普通文件',
    )
    await expect(
      deleteProjectFile(projectPath, join('.slideMind', 'private.md')),
    ).rejects.toThrow('内部文件')
    await expect(
      listProjectDirectory(projectPath, '.slideMind'),
    ).rejects.toThrow('内部目录')
    await expect(listProjectDirectory(projectPath, '.git')).rejects.toThrow(
      '内部目录',
    )
  })
  ...
  it('renames a directory and prepares old and new version paths', async () => {
    ...
    await expect(
      projectDirectoryRenamePaths(projectPath, input),
    ).resolves.toEqual({
      input,
      paths: [
    ...
    await expect(
      readFile(join(projectPath, 'published', 'nested', 'notes.txt'), 'utf8'),
    ).resolves.toBe('Notes')
  ...
  it('recursively deletes a directory and reports its versioned files', async () => {
    ...
    await writeFile(
      join(projectPath, 'obsolete', 'nested', 'second.md'),
      'Second',
    )

    await expect(
      projectDirectoryDeletePaths(projectPath, 'obsolete'),
    ).resolves.toEqual({
      path: 'obsolete',
      paths: [
    ...
    await expect(
      deleteProjectDirectory(projectPath, 'obsolete'),
    ).resolves.toBeUndefined()
  ...
  it('rejects root, internal, symbolic-link and duplicate directory operations', async () => {
    ...
    await expect(deleteProjectDirectory(projectPath, '.')).rejects.toThrow(
      '超出项目范围',
    )
    await expect(
      deleteProjectDirectory(projectPath, '.slideMind'),
    ).rejects.toThrow('内部目录')
    await expect(deleteProjectDirectory(projectPath, 'linked')).rejects.toThrow(
      '符号链接',
    )
    await expect(
      renameProjectDirectory(projectPath, {
        path: 'first',
        name: 'second',
      }),
