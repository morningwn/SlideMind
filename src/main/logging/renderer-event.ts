...
function parseContext(value: unknown): Record<string, DiagnosticContextValue> | undefined {
  ...
  for (const [key, entry] of entries) {
    ...
    if (typeof entry === 'string' && entry.length > MAX_STRING_LENGTH) return undefined
    context[key] = entry
  }
  return context
}

export function parseRendererDiagnosticEvent(value: unknown): RendererDiagnosticEvent | undefined {
  if (!isRecord(value)) return undefined
  if (Object.keys(value).some((key) => !['context', 'error', 'event', 'level'].includes(key))) {
    return undefined
  }
  if (value.level !== 'warn' && value.level !== 'error') return undefined
  if (typeof value.event !== 'string' || !eventNames.has(value.event)) return undefined
