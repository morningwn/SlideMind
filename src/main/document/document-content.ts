...
export function normalizeTikaDocument(
  entries: ReadonlyArray<Record<string, unknown>>,
): NormalizedDocument {
  const root = entries[0] ?? {}
  const content = (
    typeof root['tk:content'] === 'string' ? root['tk:content'] : ''
  )
    .replaceAll('\0', '')
    .replace(/\r\n?/g, '\n')
    .trim()
  ...
  for (const key of Object.keys(root).sort()) {
    if (
      ...
      Object.keys(metadata).length >= MAX_METADATA_KEYS
    )
      continue
  ...
  const warnings = ['文档读取不保留页面视觉布局、准确页码或全部复杂表格结构。']
  const limitKeys = Object.keys(root).filter((key) => {
    const normalized = key
      ...
      .replaceAll(':', '-')
    return (
      normalized.includes('write-limit-reached') ||
      normalized.includes('output-limit-reached')
    )
  })
  if (limitKeys.length > 0)
    warnings.push('Tika 已达到正文输出限制，返回内容不完整。')
...
export function documentChunk(
  ...
  let end = safeEndOffset(
    content,
    offset,
    Math.min(content.length, offset + maxChars),
  )
  while (
    end > offset &&
    Buffer.byteLength(content.slice(offset, end), 'utf8') > maxBytes
  ) {
    end = safeEndOffset(content, offset, end - 1)
  }
  if (end < content.length) {
    const paragraphEnd = content.lastIndexOf('\n\n', end)
    if (paragraphEnd > offset + Math.floor((end - offset) / 2))
      end = paragraphEnd + 2
  }
