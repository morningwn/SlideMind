...
function createService(parser: DocumentParser, options: { cacheBytes?: number; maxQueueLength?: number } = {}) {
...
function parsed(content: string): TikaParseResult {
  return {
    entries: [{ 'tk:content': content, 'dc:title': 'Title', private: 'hidden' }],
...
describe('DocumentReadService', () => {
  it('reads a snapshot and continues with a signed cursor without reparsing', async () => {
    ...
    const parser = new FakeParser(async () => parsed('第一段内容\n\n第二段内容'))
    const { service } = createService(parser)

    const first = await service.read(projectPath, { file: 'sample.docx', maxChars: 7 })
  ...
  it('rejects an old cursor after the source file changes', async () => {
    ...
    const { service } = createService(new FakeParser(async () => parsed('abcdefghij')))
    const first = await service.read(projectPath, { file: 'sample.docx', maxChars: 4 })
    await writeFile(file, 'version two')

    await expect(service.read(projectPath, {
      cursor: first.nextCursor,
      file: 'sample.docx'
  ...
  it('rejects a modified cursor signature', async () => {
    ...
    await expect(service.read(projectPath, { cursor: tampered, file: 'sample.docx' }))
  ...
  it('coalesces concurrent parsing and lets one waiter cancel independently', async () => {
    ...
    const cancelled = service.read(projectPath, { file: 'sample.docx' }, controller.signal)
  ...
  it('recycles the runtime when every waiter cancels an active parse', async () => {
    await writeFile(join(projectPath, 'sample.docx'), 'document')
    const parser = new FakeParser(async (_file, signal) => new Promise<TikaParseResult>((_resolve, reject) => {
      signal?.addEventListener('abort', () => reject(new Error('aborted')), { once: true })
    }))
    const { runtime, service } = createService(parser)
    const controller = new AbortController()
    const pending = service.read(projectPath, { file: 'sample.docx' }, controller.signal)
    while (parser.calls === 0) await new Promise<void>((resolve) => setImmediate(resolve))
    controller.abort()
    await expect(pending).rejects.toMatchObject({ code: 'cancelled' })
    while (runtime.restarts === 0) await new Promise<void>((resolve) => setImmediate(resolve))
    expect(runtime.restarts).toBe(1)
  })

  it('bounds the serial parsing queue', async () => {
    await Promise.all(['one.docx', 'two.docx', 'three.docx'].map((file) => (
    ...
    await expect(service.read(projectPath, { file: 'three.docx' }))
  ...
  it('rejects traversal and symbolic-link reads before parsing', async () => {
    ...
    try {
      ...
      const { service } = createService(parser)
      await expect(service.read(projectPath, { file: '../secret.docx' })).rejects.toThrow()
