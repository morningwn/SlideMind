...
function mapHttpError(status: number, body: string): DocumentReadError {
  ...
  if (message.includes('password') || message.includes('encrypted')) {
    return new DocumentReadError('password_required', '文档需要密码，当前无法读取')
  }
  if (status === 415) return new DocumentReadError('unsupported_format', '文件内容不是受支持的 DOC/DOCX')
  if (status === 408 || status === 504 || message.includes('timeout')) {
    return new DocumentReadError('timeout', 'Tika 文档解析超时')
  }
  if (status === 422 || message.includes('corrupt') || message.includes('invalid')) {
    return new DocumentReadError('corrupt_document', '文档已损坏或无法解析')
  }
  return new DocumentReadError('runtime_unavailable', `Tika 请求失败（HTTP ${status}）`)
}
...
export class TikaClient {
  ...
  async parse(
    ...
    const mimeType = (await this.request(
      `${baseUrl}/detect`,
      bytes,
      file,
      signal,
    ...
    if (!ALLOWED_MIME_TYPES.has(mimeType)) {
      throw new DocumentReadError('unsupported_format', '文件内容不是受支持的 DOC/DOCX')
    }
    ...
    try {
      ...
      throw new DocumentReadError('runtime_unavailable', 'Tika 返回了无效 JSON', { cause: error })
    }
    if (!Array.isArray(value) || value.some((entry) => !entry || typeof entry !== 'object')) {
      throw new DocumentReadError('runtime_unavailable', 'Tika 返回了无效解析结果')
    }
    const entries = value as Record<string, unknown>[]
    const content = entries[0]?.['tk:content']
    if (typeof content === 'string' && content.length > MAX_EXTRACTED_CHARS) {
      throw new DocumentReadError('runtime_unavailable', 'Tika 正文超过配置的输出限制')
    }
  ...
  private async request(
    ...
    try {
      ...
      if (timeout.aborted) throw new DocumentReadError('timeout', 'Tika 文档解析超时')
      throw new DocumentReadError('runtime_unavailable', '无法连接 Tika 运行时', { cause: error })
    }
    ...
    try {
      ...
      if (signal?.aborted) throw new DocumentReadError('cancelled', '文档读取已取消')
      if (timeout.aborted) throw new DocumentReadError('timeout', 'Tika 文档解析超时')
      throw new DocumentReadError('runtime_unavailable', '读取 Tika 响应失败', { cause: error })
    }
