...
function validateString(value: unknown, label: string, maxLength = 4096): string {
  if (
    typeof value !== 'string' ||
    !value.trim() ||
...
function validateProjectRelativePath(projectPath: string, relativePath: string): string {
  ...
  if (
    normalized.split(sep).some((segment) => segment.toLocaleLowerCase() === INTERNAL_PROJECT_DIRECTORY)
  ) {
...
async function assertNoSymbolicLinks(projectPath: string, relativePath: string): Promise<void> {
...
async function resolveNewPresentationFile(
  ...
  if (!isInsideProject(projectPath, parentPath)) throw new Error('文件路径超出项目范围')
...
export function normalizePresentationDocument(value: unknown): PresentationDocument {
...
export function createBlankPresentationDocument(titleInput?: unknown): PresentationDocument {
...
function validateCreateInput(value: unknown): CreateProjectPresentationInput {
  ...
  return {
    ...
    ...(value.title === undefined ? {} : { title: validateString(value.title, '演示文稿标题', 200) })
  }
}
...
export class ProjectPresentationStore {
  ...
  async read(projectPathInput: unknown, relativePathInput: unknown): Promise<ProjectPresentationFile> {
    const { relativePath, targetPath, size } = await resolveExistingPresentationFile(
      projectPathInput,
      relativePathInput
    )
    if (size > MAX_PRESENTATION_BYTES) throw new Error('演示文稿超过 50 MiB，无法打开')
    const bytes = await readFile(targetPath)
    if (bytes.byteLength > MAX_PRESENTATION_BYTES) throw new Error('演示文稿超过 50 MiB，无法打开')
  ...
  private async write(
    ...
    const { targetPath } = await resolveExistingPresentationFile(projectPathInput, input.path)
