...
import { createToolActivity, formatToolArguments, toolErrorDetail } from './agent-activity'

describe('agent activity', () => {
  it('recognizes reading SKILL.md as a skill invocation', () => {
    expect(createToolActivity('call-1', 'read', {
  ...
  it('extracts bounded text from failed tool results', () => {
    expect(toolErrorDetail({
      content: [{ type: 'text', text: 'permission denied' }]
    })).toBe('permission denied')
    expect(toolErrorDetail({ content: [{ type: 'image', data: 'ignored' }] }))
      .toBeUndefined()
  })
})
