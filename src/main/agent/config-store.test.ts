...
describe('AgentConfigStore', () => {
  ...
  ])('loads %s as %s and retains the existing key', async (storedId, modelId) => {
    await writeFile(configPath, JSON.stringify({
      version: 1, provider: 'deepseek', modelId: storedId, encryptedApiKey: 'dGVzdA=='
    }))
    const store = new AgentConfigStore(configPath)
    expect(await store.load()).toEqual({ modelId, apiKey: 'sk-test' })
    expect(await store.getStatus()).toMatchObject({ configured: true, modelId })
    expect(await store.save({ modelId, apiKey: '' })).toMatchObject({ configured: true, modelId })
  })

  it('rejects an unknown stored model', async () => {
    await writeFile(configPath, JSON.stringify({
      version: 1, provider: 'deepseek', modelId: 'unknown', encryptedApiKey: 'dGVzdA=='
    }))
