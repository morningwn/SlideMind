...
export interface DeckReviewIssue {
  category: 'accessibility' | 'bounds' | 'content' | 'layout' | 'performance' | 'typography'
  code: string
  elementIds?: string[]
  evidence: string
  message: string
...
const SEVERITY_ORDER: Record<DeckReviewSeverity, number> = { blocker: 0, warning: 1, info: 2 }
const CONTENT_TYPES = new Set(['chart', 'latex', 'shape', 'table', 'text'])
const REWRITE_SUPPORTED_TYPES = new Set(['image', 'line', 'shape', 'text'])
const PLACEHOLDER_PATTERN = /(?:\b(?:fixme|lorem\s+ipsum|tbd|todo|xxx)\b|点击添加|待补充|待填写|占位(?:符|文字)?|模板(?:标题|正文))/iu
...
function elementText(element: PptistElement): { html: string; text: string } {
  ...
  const shapeText = isRecord(element.text) && typeof element.text.content === 'string'
...
function rotatedBox(element: PptistElement): Box {
  ...
  const halfWidth = Math.abs(Math.cos(radians)) * width / 2 +
    Math.abs(Math.sin(radians)) * height / 2
  const halfHeight = Math.abs(Math.sin(radians)) * width / 2 +
...
function isOutside(box: Box, width: number, height: number): boolean {
  return box.left < -CANVAS_EPSILON || box.top < -CANVAS_EPSILON ||
    box.right > width + CANVAS_EPSILON || box.bottom > height + CANVAS_EPSILON
}

function isBackgroundImage(element: PptistElement, box: Box, width: number, height: number): boolean {
...
function isOpaqueCover(element: PptistElement): boolean {
  if (element.type === 'image') {
    return typeof element.src === 'string' && element.src.startsWith('data:image/')
  }
  ...
  return fill !== 'transparent' && !/^#[\da-f]{6}00$/i.test(fill) &&
    !/^rgba\([^)]*,0(?:\.0+)?\)$/.test(fill)
}
...
function estimatedTextHeight(text: string, width: number, fontSize: number, html: string): number {
  ...
  for (const line of explicitLines) {
    ...
    for (const character of line || ' ') {
      estimatedWidth += /[\u2e80-\u9fff\uf900-\ufaff]/u.test(character)
        ? fontSize
        : /\s/u.test(character) ? fontSize * 0.33 : fontSize * 0.56
    }
...
function pushIssue(target: IssueAccumulator, issue: DeckReviewIssue): void {
  ...
  const replaceIndex = target.issues.findIndex((retained) =>
    SEVERITY_ORDER[retained.severity] > SEVERITY_ORDER[issue.severity]
  )
  if (replaceIndex >= 0) target.issues[replaceIndex] = issue
}

export function reviewPresentationDocument(document: PresentationDocument): DeckReviewResult {
  ...
  const reusableImages = new Map<string, Array<{ elementId: string; slideNumber: number }>>()
  ...
  presentation.slides.forEach((slide, slideIndex) => {
    ...
    slide.elements.forEach((element, index) => {
      if (!REWRITE_SUPPORTED_TYPES.has(element.type)) unsupportedTypes.add(element.type)
      const box = element.type === 'line' ? lineBox(element) : rotatedBox(element)
      ...
      if (text) {
        ...
        if (sizes.length > 0) {
          ...
          if (
            elementHeight > 0 &&
            estimatedTextHeight(text, element.width, maximumSize, html) > elementHeight * 1.15
          ) {
            pushIssue(review, {
              category: 'typography',
              ...
              severity: 'warning',
              slideNumber,
              suggestion: '缩短文案或增大文本框，并在真实渲染中确认换行和裁切。'
            })
          }
        }
      ...
      if (isOutside(box, canvasWidth, canvasHeight)) {
        ...
        const background = isBackgroundImage(element, box, canvasWidth, canvasHeight)
        const intentionalDecoration = element.type === 'shape' && element.lock === true
        if (!background && !(intentionalDecoration && ratio < 0.5)) {
          const contentElement = CONTENT_TYPES.has(element.type) &&
          ...
          pushIssue(review, {
            ...
            slideNumber,
            suggestion: '调整元素位置或尺寸；若是刻意出血装饰，确认不承载信息后可保留。'
          })
        }
      }
      ...
      if (element.type === 'image') {
        ...
        const background = isBackgroundImage(element, box, canvasWidth, canvasHeight)
        if (source.startsWith('data:image/')) embeddedImageCharacters += source.length
        if (source && !background) {
          const uses = reusableImages.get(source) ?? []
          uses.push({ elementId: element.id, slideNumber })
          reusableImages.set(source, uses)
        }
        if (!background && !(typeof element.name === 'string' && element.name.trim())) {
          pushIssue(review, {
    ...
    const sortedContent = [...contentBoxes].sort((left, right) => left.box.left - right.box.left)
    let overlapComparisons = 0
    let overlapCheckTruncated = false
    overlapPairs:
    for (let leftIndex = 0; leftIndex < sortedContent.length; leftIndex += 1) {
      const left = sortedContent[leftIndex]
      for (let rightIndex = leftIndex + 1; rightIndex < sortedContent.length; rightIndex += 1) {
    ...
    layerPairs:
    for (const content of contentBoxes) {
      for (let coverIndex = content.index + 1; coverIndex < slide.elements.length; coverIndex += 1) {
        layerComparisons += 1
        ...
        if (contentArea === 0 || intersectionArea(content.box, coverBox) / contentArea < 0.5) continue
        pushIssue(review, {
          category: 'layout',
  ...
  return {
    ...
    status: review.summary.blocker > 0
      ? 'fail'
      : review.summary.warning > 0 ? 'needs-attention' : 'pass',
