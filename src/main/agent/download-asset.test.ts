...
describe('downloadAsset', () => {
  ...
  it('rejects insecure redirect targets before opening them', async () => {
    const projectPath = await createProject()
    const openAsset: AssetOpener = async () =>
      response(302, '', {
  ...
  it('rejects non-UTF-8 text and removes the temporary file', async () => {
    const projectPath = await createProject()
    const openAsset: AssetOpener = async () =>
      response(200, Buffer.from([0xff, 0xfe, 0xfd]), { 'content-length': '3' })
  ...
  it('rejects project escapes, internal paths, plain HTTP and URL credentials', async () => {
    ...
    await expect(
      resolveDownloadTarget(projectPath, '../outside.txt'),
    ).rejects.toThrow('下载路径超出项目范围')
    await expect(
      resolveDownloadTarget(projectPath, '.git/config'),
    ).rejects.toThrow('不能下载到受保护的项目目录')
    await expect(
      resolveDownloadTarget(projectPath, 'node_modules/package/file.js'),
    ).rejects.toThrow('不能下载到受保护的项目目录')
    await expect(
      downloadAsset({
        kind: 'text',
