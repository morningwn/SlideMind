...
import { DocumentReadError, type DocumentReadResult } from '../../shared/document'
...
interface RegisteredTool {
  ...
  execute(
    ...
    signal?: AbortSignal
  ): Promise<{ content: Array<{ text: string; type: string }>; details: unknown }>
}
...
async function register(read: (...args: never[]) => Promise<DocumentReadResult>) {
...
describe('createDocumentToolsExtension', () => {
  ...
  it('passes cancellation to the reader and exposes known failure codes', async () => {
    ...
    await expect(tool.execute('read-document', {
      file: 'protected.docx'
    }, controller.signal)).rejects.toThrow('文档读取失败（password_required）：文档需要密码')
    expect(read).toHaveBeenCalledWith('/project', {
      file: 'protected.docx'
    }, controller.signal)
  })
  ...
  it('does not hide unexpected failures', async () => {
    ...
    const tool = await register(async () => { throw failure })
