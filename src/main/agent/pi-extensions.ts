...
const WEB_TOOLS_WITH_PROXY = new Set([
  'web_search',
  'source_check',
  'fetch_content',
])
const FETCH_CONTENT_EXTERNAL_OPTIONS = [
  'auth',
  'forceClone',
  'frames',
  'model',
...
export const createWebAccessGuardExtension: ExtensionFactory = (pi) => {
  pi.on('tool_call', (event) => {
    if (!WEB_TOOLS_WITH_PROXY.has(event.toolName) || !isRecord(event.input))
      return
...
export async function preparePiExtensions(
  ...
  await writeManagedJson(join(agentDirectory, 'web-search.json'), {
    workflow: 'none',
