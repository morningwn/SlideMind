...
describe('Pi extension integration', () => {
  ...
  it('loads every pinned extension without native context-mode dependencies', async () => {
    const agentDirectory = join(
      tmpdir(),
      `slidemind-pi-extensions-${crypto.randomUUID()}`,
    )
    ...
    try {
      ...
      expect(
        extensions.extensions.some((extension) =>
          extension.path?.includes('pi-free'),
        ),
      ).toBe(false)
      expect(
        extensions.extensions.some((extension) =>
          extension.path?.includes('pi-continue'),
        ),
      ).toBe(false)
  ...
  it('preserves provider settings while enforcing headless cross-platform defaults', async () => {
    const agentDirectory = join(
      tmpdir(),
      `slidemind-pi-extension-config-${crypto.randomUUID()}`,
    )
    await preparePiExtensions(agentDirectory)
    const webSearchPath = join(agentDirectory, 'web-search.json')

    await expect(
      access(join(agentDirectory, 'extensions', 'pi-continue.json')),
  ...
  it('leaves existing pi-continue settings untouched', async () => {
    const agentDirectory = join(
      tmpdir(),
      `slidemind-pi-legacy-${crypto.randomUUID()}`,
    )
    const legacyPath = join(agentDirectory, 'extensions', 'pi-continue.json')
    await mkdir(join(agentDirectory, 'extensions'), { recursive: true })
    await writeFile(legacyPath, '{"enabled":false}\n')

    await preparePiExtensions(agentDirectory)

    expect(await readFile(legacyPath, 'utf8')).toBe('{"enabled":false}\n')
  })

  it('blocks pi-web-access paths that require system executables', async () => {
    const agentDirectory = join(
      tmpdir(),
      `slidemind-web-guard-${crypto.randomUUID()}`,
    )
    const {
      createAgentSession,
  ...
  it('activates four pure-JavaScript web tools and the managed downloader', () => {
    expect(PI_AGENT_TOOL_NAMES).toEqual([
      ...PI_WEB_TOOL_NAMES,
      'download_asset',
    ])
  })
